...
let city, locationKey, useCelcius = true;
//#region ***  DOM references ***
let htmlBody, htmlDarkModebtn, htmlCity, htmlWeatherIcon, htmlWeatherPhrase, htmlTemp, htmlRealTemp, htmlUVIndex, htmlAQI, htmlWindSpeed, htmlWindSpeedType
//#endregion
...
let getWeather = (jsonLocation) => {
    ...
    locationKey = jsonLocation.Key;
    // get weather from api and return to callback
    fetch(`https://dataservice.accuweather.com/forecasts/v1/daily/1day/${jsonLocation.Key}?apikey=${API_KEY}&details=true&metric=${useCelcius}`)
        .then(response => response.json())
        .then(jsonWeather => showForecast(jsonWeather))
}

let reloadWeather = () => {
    htmlWindSpeedType.childNodes[1].nodeValue = useCelcius ? 'km/h' : 'mph';
    fetch(`https://dataservice.accuweather.com/forecasts/v1/daily/1day/${locationKey}?apikey=${API_KEY}&details=true&metric=${useCelcius}`)
        .then(response => response.json())
        .then(jsonWeather => showForecast(jsonWeather))
}
//#endregion
...
let showForecast = (jsonWeather) => {
    ...
    htmlTemp.childNodes[0].nodeValue = Math.round(jsonWeather.DailyForecasts[0].Temperature.Maximum.Value).toString();
    ...
    htmlWindSpeedType.childNodes[0].nodeValue = useCelcius ? 'km/h' : 'mph';
...
let listenToSettings = () => {
    // listen to settings button
    document.querySelector('#js-popup-button').addEventListener('click', () => {
        // toggle settings panel
        document.querySelector('#js-popup').classList.toggle("show");
    })
    ...
    // celsius
    document.querySelector('.js-celsius').addEventListener('click', () => {
        // change state and reload weather
        useCelcius = true;
        reloadWeather();
    });
    // fahrenheit
    document.querySelector('.js-fahrenheit').addEventListener('click', () => {
        useCelcius = false;
        reloadWeather();
    });
...
let setBackground = (weather) => {
    // if dark mode isn't on, set background colors based on weather
    if (!htmlBody.classList.contains('dark-mode')) {
        htmlBody.className = '';
        htmlBody.classList.add(weather);
    }
}

let setWeather = (weatherType) => {
    if (weatherType.includes('rain') || weatherType.includes('shower')) {
        setBackground('is-rainy');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    htmlWindSpeedType = document.querySelector('.js-windSpeedType');
